...
export function buildPinataMultipartBody({
  ...
  const filename = createSafeUploadFilename(mimeType);
  const metadataJson = JSON.stringify({ name: filename });
  ...
  const bodyBuffer = Buffer.concat([fileHeader, Buffer.from(fileBytes), metadataHeader]);
