...
export function buildPinataMultipartBody({
  fileBytes,
  mimeType,
  filename,
}) {
  const boundary = `----tronlaunchpad-${Date.now().toString(16)}-${Math.random().toString(16).slice(2, 10)}`;
  const safeFilename = sanitizeHeaderFilename(filename, mimeType);
  const metadataJson = JSON.stringify({ name: safeFilename });
  ...
  const body = Buffer.concat([fileHeader, Buffer.from(fileBytes), metadataHeader]);
